...
- [x] Add `npm audit --audit-level=high` to ci.yml (replace the `--omit=dev` step; the vulns come via devDeps)
- [x] Add `npm audit --audit-level=high` as a Dockerfile build step (dedicated stage or RUN)
- [x] Update SECURITY.md wording to match CI behavior
